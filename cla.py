...
class Inference(object):
    ...
    def __init__(self, model_rpn = None, model_classifier = None, model_path = None, num_features = None,
        input_shape_img = None, input_shape_features = None):
    ...
    def __set_img_format(self, num_features = None):
    ...
    # 格式化图片大小
    def __format_img_size(self, img, im_size = None):
        im_size = 600
    ...
    # 根据配置格式化图像通道
    def __format_img_channels(self, img, img_channel_mean = None, img_scaling_factor = None):
    ...
    def create_model(self, anchor_box_ratios = None, anchor_box_scales = None):
        ...
        classifier = nn.classifier(feature_map_input, roi_input, self.num_rois, nb_classes=len(self.class_mapping), trainable=True)
        model_rpn = Model(img_input, rpn_layers)
        model_classifier = Model([feature_map_input, roi_input], classifier)

        return model_rpn, model_classifier

    def load_model(self, model_path = None, model_rpn = None, model_classifier = None):
        print('loading model')
        # 加载模型
        print('Loading weights from {}'.format(model_path))
        model_path = self.model_path
        model_rpn, model_classifier = self.create_model()
    ...
    def pre_predict(self, pic_path = None, bbox_threshold = None, model_rpn = None, model_classifier = None, 
        classifier_regr_std = None, rpn_stride = None):
    ...
    def post_predict(self, ratio = None, bboxes = None, probs = None):
        print('confirming location')
        # 求出预测框的比例前原始坐标和真实坐标
        # origin_boxes = []
        real_boxes = []
        bboxes, probs, ratio= self.pre_predict()
        for key in bboxes:
